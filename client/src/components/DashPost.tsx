...
export default function DashPosts() {
  ...
  useEffect(() => {
    if (!currentUser?.isAdmin) {
      navigate('/notFound'); // Redirect non-admin users
    }
  ...
  return (
    <div className="py-5 px-4 sm:px-8 md:px-20 overflow-x-hidden">
      {currentUser.isAdmin && userPosts.length > 0 ? (
        <>
          ...
          <div className="block md:hidden space-y-4 py-15">
            {userPosts.map((post) => (
              <div key={post._id} className="border rounded-lg p-4 shadow-sm bg-white dark:bg-gray-800">
                ...
                <Link to={`/post/${post.slug}`} className="flex gap-4 items-start">
                  <img
                    src={post.image}
                    alt={post.title}
                    className="w-20 h-12 object-cover rounded bg-gray-300"
                  />
                ...
                <div className="flex justify-between items-center mt-4 text-sm">
                  ...
                  <Link to={`/update-post/${post._id}`} className="text-green-600 hover:underline">
                    Edit
                  </Link>
                </div>
              </div>
